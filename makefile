...
$(EXEC): $(LEX_O) $(UTIL_O) $(SYMTAB_O) $(SEMANTICO_O) $(GERADORDEQUADRUPLA_O) $(GERADORASSEMBLY_O) $(YACC_C)
	g++ -o $(EXEC) $(YACC_C) $(UTIL) $(LEX_O) $(SYMTAB) $(SEMANTICO) $(GERADORDEQUADRUPLA) $(GERADORASSEMBLY) -lfl

run: $(EXEC)
	./$(EXEC)

clean:
	rm -f $(LEX_C) $(YACC_C) $(YACC_H) $(LEX_O) $(UTIL_O) $(SYMTAB_O) $(SEMANTICO_O) $(GERADORDEQUADRUPLA_O) $(GERADORASSEMBLY_O) $(EXEC)
